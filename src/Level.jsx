...
function BlockSpinner({ position = [0, 0, 0] }) {
    ...
    /* 
       De snelheid van een spinner wordt willekeurig aangemaakt bij het aanroepen van deze functie. 
       Om te voorkomen dat die veels te traag is doen we altijd een 0.2 erop. Als de getal kleiner is 
       dan 0.5 dan gaat de spinner de andere kant op draaienm, zo niet dan gaat die klokwijze draaien.
    */
    const [speed] = useState(() => (Math.random() + 0.2) * (Math.random() < 0.5 ? - 1 : 1))

    // De Z as wordt bijgewerkt om het object te draaien.
    useFrame((state) => {
...
function BlockLimbo({ position = [0, 0, 0] }) {
    ...
    // De Y as wordt bijgewerkt om het object boven en beneden te plaatsen.
    useFrame((state) => {
        const time = state.clock.getElapsedTime()
...
function BlockAxe({ position = [0, 0, 0] }) {
    ...
    // De X as wordt bijgewerkt om het object van links naar rechts te plaatsen.
    useFrame((state) => {
        const time = state.clock.getElapsedTime()
...
export default function Level({ count = 5, types = [BlockSpinner, BlockAxe, BlockLimbo], seed = 0 }) {
    ...
    // useMemo wordt hier gebruikt om alleen veranderingen in de count en types te verwerken en niet onnodige re-renders te maken.
